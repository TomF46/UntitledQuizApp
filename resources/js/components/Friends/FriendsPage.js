...
import { getFriendsList, getFriendRequestsList, removeOrRejectFriend, acceptFriendRequest } from "../../api/friendsApi";
...
const FriendsPage = ({ history }) => {
    ...
    function getFriends() {
        getFriendsList().then(friendsData => {
    ...
    function handleFriendListPageChange(url) {
        getPageWithPaginationUrl(url).then(friendsData => {
            setFriendsPaginator(friendsData);
        }).catch(error => {
            toast.error(`Error getting friends ${error.message}`, {
                autoClose: false,
            ...
            <div className="grid grid-cols-12">
                ...
                <div className="col-span-12 lg:col-span-9">
                    <div className="mb-6 overflow-hidden shadow card px-4 py-2">
                        <h1 className="font-bold text-primary text-primary text-2xl text-center md:text-left">Friend Requests</h1>
                    ...
                    <div className="overflow-hidden shadow card px-4 py-2">
                        <h1 className="font-bold text-primary text-primary text-2xl text-center md:text-left">Friends</h1>
                        {!friendsPaginator ? (
                            <LoadingMessage message={'Loading friends'} />
                        ) : (

                            friendsPaginator.total > 0 ? (
                                < FriendsListWithPagination paginationData={friendsPaginator} onPageChange={handleFriendListPageChange} onRemove={handleRemoveOrRejectFriend} />
                            ) : (
