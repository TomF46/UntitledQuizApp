...
const QuestionManagement = ({
  ...
  function onAddAnswer(questionIndex) {
    updateQuiz(
      QuizManagementService.addBlankAnswerToQuestion(quiz, questionIndex),
    );
  }
  ...
  function onQuestionChange(questionIndex, event) {
    ...
    updateQuiz(
      QuizManagementService.changeQuestionText(quiz, questionIndex, value),
    );
  }
  ...
  function onQuestionHelpTextChange(questionIndex, event) {
    const { value } = event.target;
    updateQuiz(
      QuizManagementService.changeQuestionHelpText(quiz, questionIndex, value),
    );
  }
  ...
  function onAnswerChange(questionIndex, answerIndex, event) {
    const { value } = event.target;
    updateQuiz(
      QuizManagementService.changeAnswerText(
        quiz,
        questionIndex,
        answerIndex,
        value,
  ...
  function onAnswerCheckboxChange(questionIndex, answerIndex, event) {
    ...
    updateQuiz(
      QuizManagementService.setIsCorrectForAnswer(
        quiz,
        questionIndex,
        answerIndex,
        checked,
  ...
  function onQuestionVideoChange(questionIndex, event) {
    const { value } = event.target;
    updateQuiz(
      QuizManagementService.changeQuestionVideo(quiz, questionIndex, value),
    );
  }

  function onQuestionVideoRemoved(questionIndex) {
    updateQuiz(
      QuizManagementService.changeQuestionVideo(quiz, questionIndex, null),
    );
    closeModal();
  }
  ...
  function onQuestionImageChange(questionIndex, event) {
    ...
    storeImage(file)
      .then((res) => {
        ...
        updateQuiz(
          QuizManagementService.changeQuestionImage(
            quiz,
            questionIndex,
            res.path,
          ),
  ...
  function onRemoveImage(questionIndex) {
    updateQuiz(
      QuizManagementService.changeQuestionImage(quiz, questionIndex, null),
    );
  }
      ...
      buttons: [
        {
          ...
          onClick: () => {
            updateQuiz(
              QuizManagementService.removeQuestion(quiz, questionIndex),
            );
          },
        },
        {
      ...
      buttons: [
        {
          ...
          onClick: () => {
            updateQuiz(
              QuizManagementService.removeAnswer(
                quiz,
                questionIndex,
                answerIndex,
              ),
  ...
  return (
    <>
      {quiz.questions.map((question, questionIndex) => {
        return (
          <div
            className='p-4 shadow card mt-4'
            key={question.id ? question.id : questionIndex}
          >
            ...
            <div className='flex mt-2'>
                  ...
                  >
                    ...
                    <label className='pointer ml-1'>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          onQuestionImageChange(questionIndex, e)
                        }
                      />
                    </label>
                  </button>
              ...
              <div>
                <button
                  type='button'
                  onClick={openModal}
                  className='bg-primary text-white rounded py-2 px-4 ml-2 hover:opacity-75 shadow inline-flex items-center'
                >
                  <svg
                    xmlns='http://www.w3.org/2000/svg'
                    fill='none'
                ...
                >
                  <div className='grid grid-cols-12'>
                    ...
                    <div className='col-span-12'>
                      <p>
                        Please add a valid embed link for your video on your
                        format of choice in the input below, if you add a valid
                        link then a preview of the video will appear below.
                      </p>
                      <br />
                      <p>Example links include</p>
                      <ul>
                        <li>
                          Youtube: https://www.youtube.com/embed/5mGuCdlCcNM
                        </li>
                    ...
                    <div className='col-span-12 mt-4'>
                      <TextInput
                        ...
                        onChange={(e) =>
                          onQuestionVideoChange(questionIndex, e)
                        }
                        required={true}
                      />
                    </div>
                    ...
                    <div className='col-span-12 text-right'>
                      <button
                        type='button'
                        onClick={() => {
                          onQuestionVideoRemoved(questionIndex);
                        }}
            ...
            {quiz.questions[questionIndex].image_url != null && (
              <img
                src={quiz.questions[questionIndex].image_url}
                alt='image-preview'
                className='question-image-preview mt-4'
              />
            )}
            ...
            {quiz.questions[questionIndex].answers.length > 0 && (
              <h2 className='font-bold text-primary text-lg mt-2'>Answers:</h2>
            )}
            {quiz.questions[questionIndex].answers.map(
              (answer, answerIndex) => {
                return (
                  <div
                    className='mb-2'
                    key={answer.id ? answer.id : answerIndex}
                  >
                    <TextInput
                      ...
                      onChange={(e) =>
                        onAnswerChange(questionIndex, answerIndex, e)
                      }
                      required={true}
                    />
                    <div className='flex justify-between'>
                      <div>
                        <CheckboxInput
                          ...
                          value={
                            quiz.questions[questionIndex].answers[answerIndex]
                              .is_correct
                          }
                          checked={
                            quiz.questions[questionIndex].answers[answerIndex]
                      ...
                      <div>
                        <p
                          className='inline text-red-400 font-bold text-sm pointer hover:text-red-500'
                          onClick={() =>
