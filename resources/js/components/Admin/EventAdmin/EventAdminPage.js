...
const EventsAdminPage = () => {
    ...
    function getEventsPage(url) {
        getPageWithPaginationUrl(url).then(eventsData => {
            setEventsPaginator(eventsData);
        }).catch(error => {
            toast.error(`Error getting events ${error.message}`, {
                autoClose: false,
            ...
            <div className="grid grid-cols-12 pb-4">
                ...
                <div className="col-span-12 lg:col-span-9 overflow-hidden shadow page px-4">
                    {!eventsPaginator ? (
                        ...
                        <>
                            <h1 className="font-bold text-primary text-4xl my-4 text-center">Events</h1>
                            {eventsPaginator.total > 0 ? (
                                <EventsListWithPagination paginationData={eventsPaginator} onPageChange={getEventsPage} />
                            ) : (
                                <p className="text-center">There are currently no events added.</p>
                            )}
                        </>
                    )}
                </div>
