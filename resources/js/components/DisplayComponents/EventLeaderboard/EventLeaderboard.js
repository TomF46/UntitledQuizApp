...
const EventLeaderboard = ({ event }) => {
    ...
    function getScores() {
        getEventLeaderboard(event.id).then(scores => {
            setScores(scores);
        }).catch(error => {
    ...
    return (
        <div className="inline-block p-4 min-w-full">
            <h2 className="font-bold text-primary text-2xl mb-4 text-center">{event.name} leaderboard</h2>
            {!scoresPaginator ? (
                <LoadingMessage message={'Loading scores'} />
            ) : (
                <EventLeaderboardTableWithPagination paginationData={scoresPaginator} onPageChange={getScoresPage} />
            )}
        </div>
    );
};
