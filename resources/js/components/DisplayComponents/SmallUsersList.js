...
const SmallUsersList = ({ users }) => {
    return (
        <div>
            {users.map((user) => {
                return (
                    <div key={user.id} className="grid grid-cols-12 p-1 border-b border-gray-200 overflow-hidden">
                        ...
                        <div className="col-span-8 flex">
                            <Link to={`/profile/${user.id}`} className="font-bold text-secondary pointer vertical-centered-text">{user.username}</Link>
                        </div>
                    </div>
                )
            })}
